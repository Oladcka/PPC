    ...
    try {
        if (system.contains("Яндекс")) {
            ...
            if (checkYandexReport(workbookNew)) {
                String code = workbookNew.getSheetAt(0).getRow(5).getCell(10).toString().substring(8, 11);
                for (int i = 5; i < workbookNew.getSheetAt(0).getPhysicalNumberOfRows(); i++) {
    ...
    private String[] removeArticles (String[] text) {
       String[] articleList = {"в", "за", "на", "до", "к", "под", "через", "для", "по", "от", "у", "и", "или", "с"};
        ...
        for (String word : text) {
            if (!Arrays.asList(articleList).contains(word)) {
                resultList.add(word);
            }
        }

// Преобразование списка обратно в массив
        text = resultList.toArray(new String[0]);
        return text;
    }
